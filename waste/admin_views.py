...
class add_product(TemplateView):
    ...
    def post(self,request,*args, **kwargs):
        # Make sure form has proper validation attributes
        ...
        # Create product with status=1 (available)
        product = products()
        product.name = name
        product.desc = desc
        product.rate = rate
...
class CollectorList(LoginRequiredMixin, ListView):
    model = collector_Registration
    template_name = 'admin/collector_list.html'
    context_object_name = 'collectors'

    def get_queryset(self):
        return collector_Registration.objects.all().select_related('collector_id')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Collector List'
        return context
...
class add_collector(TemplateView):
    template_name = 'admin/add_collector.html'
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            collector.status = 1  # Active status since added by admin
            collector.save()
            ...
            messages.error(request, f"Failed to add collector: {str(e)}")
            return render(request, self.template_name)

class view_collectors(TemplateView):
    template_name = 'admin/view_collectors.html'
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        collectors = collector_Registration.objects.all()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            collector = collector_Registration.objects.get(id=collector_id)
            
            if action == 'approve':
                collector.status = 1
                messages.success(request, f"Collector {collector.name} has been approved")
            elif action == 'reject':
                collector.status = 2
                messages.success(request, f"Collector {collector.name} has been rejected")
            elif action == 'delete':
                collector.collector_id.delete()  # This will also delete the collector registration due to CASCADE
                messages.success(request, f"Collector {collector.name} has been deleted")
                return redirect('admin_view_collectors')
...
def approve_collector(request, pk):
    collector = get_object_or_404(collector_Registration, pk=pk)
    collector.status = 1
    collector.save()
    messages.success(request, f'Collector {collector.name} has been approved')
    return redirect('admin_collector_list')

def reject_collector(request, pk):
    collector = get_object_or_404(collector_Registration, pk=pk)
    collector.status = 2
    collector.save()
    messages.success(request, f'Collector {collector.name} has been rejected')
...
def delete_collector(request, pk):
    collector = get_object_or_404(collector_Registration, pk=pk)
    collector.collector_id.delete()  # This will also delete the collector due to CASCADE
    messages.success(request, f'Collector {collector.name} has been deleted')
    return redirect('admin_collector_list')
